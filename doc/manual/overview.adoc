...
** `RAFT`: implements the Raft algorithm, ie. appending entries to the log, committing them, syncing new members etc
** `CLIENT`: handles client requests and redirects requests to the leader
...
The components are discussed later.