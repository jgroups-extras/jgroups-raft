 ...
 */
public sealed interface StateMachineWrapper<T> extends StateMachine {
     ...
     */
    void initialize(Settable settable);
     ...
     */
    <O> O submit(Function<T, O> function, JGroupsRaftCommandOptions options);

    static <T> StateMachineWrapper<T> create(T concrete, Class<T> api, CommandRegistry<T> registry, Serializer serializer) {
        if (concrete instanceof AsyncSnapshot) {
            return new Internal.AsynchronousStateMachineWrapper<>(concrete, api, registry, serializer);
        }

        return new Internal.SynchronousStateMachineWrapper<>(concrete, api, registry, serializer);
    }

    final class Internal {
        private static sealed abstract class BaseStateMachine<T> implements StateMachineWrapper<T> {
             ...
             */
            private BaseStateMachine(T concrete, Class<T> api, CommandRegistry<T> registry, Serializer serializer) {
            ...
            @Override
            public final void initialize(Settable settable) {
            ...
            @Override
            public final <O> O submit(Function<T, O> function, JGroupsRaftCommandOptions options) {
             ...
             */
            @Override
            public final byte[] apply(byte[] data, int offset, int length, boolean serialize_response) {
            ...
            @Override
            public final void writeContentTo(DataOutput out) throws Exception {
                delegate.writeContentTo(out);
            }
        }

        private static final class SynchronousStateMachineWrapper<T> extends BaseStateMachine<T> {
             ...
             * @param serializer The serializer used to convert commands to/from byte arrays for Raft replication.
             */
            private SynchronousStateMachineWrapper(T concrete, Class<T> api, CommandRegistry<T> registry, Serializer serializer) {
                super(concrete, api, registry, serializer);
            }
        }

        private static final class AsynchronousStateMachineWrapper<T> extends BaseStateMachine<T> implements AsyncSnapshot {

            private final AsyncSnapshot delegate;
             ...
             * @param serializer The serializer used to convert commands to/from byte arrays for Raft replication.
             */
            private AsynchronousStateMachineWrapper(T concrete, Class<T> api, CommandRegistry<T> registry, Serializer serializer) {
                super(concrete, api, registry, serializer);
                this.delegate = (AsyncSnapshot) concrete;
            }
