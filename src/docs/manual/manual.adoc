= Distributed consensus with JGroups Raft
:author: Bela Ban <belaban@yahoo.com>
:toc2:
:toclevels: 3
:icons:
:homepage: https://jgroups-extras.github.io/jgroups-raft
:source-highlighter: pygments
:iconsdir: ./images/icons

&copy; Red Hat 2014 – 2025

This document is licensed under the
http://creativecommons.org/licenses/by-sa/3.0/us/legalcode["Creative Commons Attribution-ShareAlike (CC-BY-SA) 3.0"]
license.


This is the JGroups Raft manual. It provides information about

* Design and architecture
* Configuration and use

of jgroups-raft.

Bela Ban, Kreuzlingen Switzerland 2017


include::./overview.adoc[Overview]

include::./using.adoc[Using jgroups-raft]

include::./blocks.adoc[Building blocks]

include::./log-integrity.adoc[Log Integrity and Recovery]

include::./tutorials/index.adoc[Tutorials]

include::./howto/index.adoc[How-to guides]

include::./reference/index.adoc[Reference]

include::./explanation/index.adoc[Explanation]

include::./protocols-generated.adoc[Protocol list]

include::./migration.adoc[Migration guide]
